...
RUN sed -i "s/user = www-data/user = root/" /etc/php/7.2/fpm/pool.d/www.conf \
    ...
    && sed -i '/^;clear_env = no/s/^;//' /etc/php/7.2/fpm/pool.d/www.conf \
    && sed -i '/^;ping\.path/s/^;//' /etc/php/7.2/fpm/pool.d/www.conf \
    && sed -i '/^;pm\.status_path/s/^;//' /etc/php/7.2/fpm/pool.d/www.conf \
    && sed -i '/.*xdebug.so$/s/^/;/' /etc/php/7.2/mods-available/xdebug.ini
